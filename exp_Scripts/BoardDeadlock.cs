...
public class BoardDeadlock : MonoBehaviour
{
    ...
    List<GamePiece> GetMinimumMatches(List<GamePiece> gamePieces, int minForMatch = 2)
    {
        ...
        var groups = gamePieces.GroupBy(n => n.m_matchValue);
    ...
    bool HasMoveAt(GamePiece[,] allPieces, int x, int y, int listLength = 3, bool checkRow = true)
    {
        ...
        if (pieces != null && matches != null)
        {
            ...
            {
                List<GamePiece> neighbors = GetNeighbors(allPieces, unmatchedPiece.m_xIndex, unmatchedPiece.m_yIndex);
                neighbors = neighbors.Except(matches).ToList();
                neighbors = neighbors.FindAll(n => n.m_matchValue == matches[0].m_matchValue);
            ...
            if (matches.Count >= listLength)
            {
                ...
                //Debug.Log("Move " + matches[0].m_matchValue + " piece to " + unmatchedPiece.m_xIndex + "," + 
                //    unmatchedPiece.m_yIndex + " to form matching " + rowColStr);
                return true;
            }
        }
